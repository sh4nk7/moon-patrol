    ...
    def collide(self, other):
        if isinstance(other, Rock) :
            ...
            if  x <= self._x + self._w // 2 <= x+w and y <=self._y + self._h  // 2 <= y+h:
                ...
                self._rewind =g2d.confirm("GAME OVER! I'M SORRY!\n Vuoi ricominciare?")
        ...
        if isinstance(other, Hole) :
            x, y, w, h = other.position()
            if  x <= self._x + self._w // 2 <= x+w and y <=self._y + self._h  // 2 <= y+h:
                g2d.draw_image_clip(sprites, (82,153,29,27) , hole.position())
                self._rewind =g2d.confirm("GAME OVER! I'M SORRY!\n Vuoi ricominciare?")
                arena_w, arena_h = self._arena.size()
                if self._rewind:
                    self._y = arena_h - self._h
...
class Rock(Actor):
    def __init__(self, arena, x, y, s):
        ...
        self._s = s
        self._dx, self._dy = self._speed, self._speed
        self._difficolta = 0.5 # random.randrange(1,2)
        self._arena = arena
        arena.add(self)
...
class Bullet(Actor):
    ...
    def move(self):
        ...
        self._y += self._dy
        if self._y < 0 or self._y > arena_h:
            self._arena.remove(self)
    ...
    def symbol(self):
        return 182, 232, self._w, self._h

    def collide(self, other):
    	if isinstance(other, Alien) or isinstance(other, Rock) or isinstance(other, Tank):
...
def tick():
    ...
    if g2d.key_pressed("ArrowUp"):
        ...
        rover_posx, rover_posy, rover_w, rover_h = rover.position()
        bullet = Bullet(arena, rover_posx + 10, rover_posy)
        bullet.go_up()
    elif g2d.key_pressed("KeyD"):
        rover_posx, rover_posy, rover_w, rover_h = rover.position()
        bullet = Bullet(arena, rover_posx + 35, rover_posy + 10)
        bullet.go_right()
    ...
    for a in arena.actors():
        ...
        if isinstance(a, Alien) and random.random() <= 0.02:
            a_posx, a_posy, a_w, a_h = a.position()
            bullet = Bullet(arena, a_posx, a_posy + 20)
            bullet.go_down()
